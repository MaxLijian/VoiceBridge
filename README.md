...
- **Browser & web app compatible** — pre-routes all major browsers (Chrome, Safari, Arc, Edge, Firefox, Brave, Opera, Zen) to clipboard+paste, ensuring `contenteditable` / React / Vue inputs receive proper DOM `paste` events
...
### Feishu WebSocket Connection Fix

Fixed a bug where VoiceBridge would perpetually show "Connecting" and never establish a connection.

#### Root Cause

The Feishu endpoint API returns the WebSocket URL as `"URL"` (uppercase), but the client was only looking for `"url"` (lowercase). The URL was always read as an empty string, causing the connection to throw `invalidResponse` on every attempt.

#### Fix

Updated `fetchEndpoint()` to accept both `"url"` and `"URL"`:
...
### Browser Web Input Fix (Gemini, ChatGPT, Claude, etc.)

Fixed text injection in browser `contenteditable` inputs (React/Vue apps like Gemini, ChatGPT, Claude web, Notion, etc.).

#### Root Cause

The Accessibility API can write text into a `contenteditable` div, but does **not** fire DOM `input` or `paste` events. React/Vue/Angular-based web inputs only update their state in response to these events — so text written via AX is silently ignored by the framework.

#### Fix

Pre-route all major browsers to the clipboard + AppleScript `keystroke "v"` path. This triggers a real browser `paste` event, which web frameworks handle correctly. The browser's address bar (native UI) also accepts paste, so there is no regression.

**Covered browsers:** Chrome, Safari, Arc, Edge, Firefox, Brave, Opera, Zen
...
| Browser address bar | ✅ clipboard | ✅ clipboard |
| Gemini / ChatGPT / Claude web | ✅ clipboard | ✅ clipboard |
| Notion / Linear (web) | ✅ clipboard | ✅ clipboard |
...
- **浏览器全兼容**（Chrome、Safari、Arc、Edge、Firefox、Brave、Opera、Zen），统一走剪贴板 + paste 事件，确保 contenteditable / React / Vue 输入框正常响应
...
修复了 VoiceBridge 始终显示"连接中"、永远无法建立连接的 Bug。

#### 根本原因

飞书 endpoint API 返回的 WebSocket URL 字段名为 `"URL"`（大写），但客户端代码只查找 `"url"`（小写）。导致 URL 始终被读取为空字符串，每次尝试都抛出 `invalidResponse` 异常。

#### 修复

在 `fetchEndpoint()` 中同时兼容两种大小写：
...
### 浏览器 Web 输入框修复（Gemini、ChatGPT、Claude 等）

修复了在浏览器 `contenteditable` 输入框（React/Vue 框架，如 Gemini、ChatGPT、Claude 网页版、Notion 等）中文字无法注入的问题。

#### 根本原因

Accessibility API 可以向 `contenteditable` div 写入文字，但**不会触发** DOM `input` 或 `paste` 事件。React/Vue/Angular 类 Web 输入框只在收到这些事件时才更新状态，所以 AX 写入的内容会被框架静默忽略。

#### 修复

将主流浏览器也加入强制剪贴板通道列表，改走剪贴板 + AppleScript `keystroke "v"` 路径。这会触发真实的浏览器 `paste` 事件，Web 框架可正常响应。浏览器地址栏（原生 UI）同样支持粘贴，不存在退化。

**覆盖浏览器：** Chrome、Safari、Arc、Edge、Firefox、Brave、Opera、Zen

#### 兼容性矩阵（更新）

| 应用 | 英文 | 中文/CJK |
|------|------|----------|
| 备忘录 / Notes | ✅ AX API | ✅ AX API |
| 浏览器地址栏 | ✅ 剪贴板 | ✅ 剪贴板 |
| Gemini / ChatGPT / Claude 网页 | ✅ 剪贴板 | ✅ 剪贴板 |
| Notion / Linear（网页）| ✅ 剪贴板 | ✅ 剪贴板 |
| 终端 Terminal | ✅ keystroke | ✅ 剪贴板 |
| VS Code 编辑器 | ✅ 剪贴板 | ✅ 剪贴板 |
| 豆包 / Doubao | ✅ 剪贴板 | ✅ 剪贴板 |
| 飞书桌面版 | ✅ 剪贴板 | ✅ 剪贴板 |
...
### VS Code 与 WebView 类应用注入修复

修复了在 VS Code 和豆包等 WebView 类应用中文字无法注入到光标处的问题。

#### 根本原因

1. **VS Code 中 AX API 假成功** — Accessibility API 接受了 `kAXSelectedTextAttribute` 写入并返回成功，但文字实际上不出现在编辑器中。现在对 VS Code 家族（含 Insiders / VSCodium）提前走剪贴板通道，跳过 AX 尝试。

2. **Electron/WebView 剪贴板竞态** — `NSPasteboard.setString` 之后立即触发 Cmd+V，粘贴事件比剪贴板变更更早到达目标进程。修复：写入剪贴板后延迟 80ms 再发送粘贴事件，确保 Electron 渲染进程能读到新内容。

3. **粘贴事件路由** — 从向 `.cghidEventTap`（全局 HID 队列）发送改为 `CGEvent.postToPid(_:)`，直接投递到前台应用的 PID，避免在多窗口场景中被错误的窗口消费。
